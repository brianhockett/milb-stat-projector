...
for year in years:
    ...
    if data:
        df = pd.DataFrame(data, columns = headers)
        filename = os.path.join(data_dir, f'pitcher_{year}.csv')
        df.to_csv(filename, index = False)
        print(f'Minor League Pitcher Data for {year} Saved to {filename}\n')
    else:
        print(f'No Minor League Pitcher Data for {year}\n')    
